...
(function () {
  ...
  var LS_CRT_ITGOA   = 'tcp_crt_ITGOA';
  ...
  function aggiornaStato() {
    ...
    try {
      var _totaleCRT = 0;
      ['tcp_crt_ITLIV','tcp_crt_ITSPE','tcp_crt_ITGOA'].forEach(function(k){
  ...
  function apriTariffarioCRT() {
    ...
    // Legge CRT diviso per porto (3 chiavi separate per evitare quota 5MB)
    var rows = [];
    try {
      ['ITLIV','ITSPE','ITGOA'].forEach(function(pc) {
        var k = 'tcp_crt_' + pc;
      ...
      // fallback legacy: legge da tcp_listino_base se chiavi nuove vuote
      if (!rows.length) {
        var rawLeg = localStorage.getItem(LS_LISTINO_BASE);
        if (rawLeg) rows = JSON.parse(rawLeg).rows || [];
      }
    } catch(e) {}
  ...
  function apriPopup(risultati){
    ...
    try {
      ['tcp_crt_ITLIV','tcp_crt_ITSPE','tcp_crt_ITGOA'].forEach(function(k){
        var _r = localStorage.getItem(k);
        if (_r) { var _p = JSON.parse(_r); _crtRows = _crtRows.concat(_p.rows||[]); }
      });
      // fallback legacy
      if (!_crtRows.length) {
