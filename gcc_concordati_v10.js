...
(function () {
  ...
  function apriGestioneVettori() {
    ...
    // Espone callback per aggiornare dati in memoria
    window._gccOnVettoriSaved = function(data) {
      try { localStorage.setItem(LS_VETTORI_REG, JSON.stringify(data)); } catch(e) {}
    };

    var _html = "<!DOCTYPE html><html><head><meta charset='UTF-8'><title>Gestione Vettori</title><style>" + "\n*{box-sizing:border-box;margin:0;padding:0;font-family:Arial,sans-serif}\nbody{background:#f0f3f6;height:100vh;display:flex;flex-direction:column;overflow:hidden}\n#topbar{display:flex;align-items:center;gap:8px;background:linear-gradient(135deg,#6c3483,#9b59b6);color:white;padding:9px 16px;flex-shrink:0}\n#topbar h2{font-size:14px;font-weight:bold;flex:1}\n.tbtn{padding:6px 13px;border:none;border-radius:5px;cursor:pointer;font-size:12px;font-weight:bold;color:white}\n#main{display:flex;flex:1;overflow:hidden}\n/* Sidebar */\n#sidebar{width:210px;background:white;border-right:1px solid #e0e0e0;display:flex;flex-direction:column;flex-shrink:0}\n#sidebar-header{padding:10px 12px;border-bottom:1px solid #eee;font-size:11px;font-weight:bold;color:#888;display:flex;align-items:center;justify-content:space-between}\n#btn-nuovo{padding:3px 10px;background:#9b59b6;color:white;border:none;border-radius:4px;cursor:pointer;font-size:11px;font-weight:bold}\n#vlist{flex:1;overflow-y:auto}\n.vitem{padding:9px 12px;cursor:pointer;border-bottom:1px solid #f5f5f5;display:flex;flex-direction:column;gap:3px}\n.vitem:hover{background:#f8f0ff}\n.vitem.active{background:#ede0f8;border-left:3px solid #9b59b6}\n.vname{font-size:13px;font-weight:bold;color:#333}\n.vbadges{display:flex;gap:4px;flex-wrap:wrap}\n.badge{padding:1px 6px;border-radius:10px;font-size:10px;font-weight:bold;color:white}\n.badge-spe{background:#2980b9}\n.badge-liv{background:#27ae60}\n.badge-km{background:#e67e22}\n/* Editor */\n#editor{flex:1;overflow-y:auto;padding:20px}\n#editor.empty{display:flex;align-items:center;justify-content:center;color:#bbb;font-size:14px}\n.card{background:white;border-radius:10px;padding:16px;margin-bottom:14px;box-shadow:0 2px 8px rgba(0,0,0,.07)}\n.card-title{font-size:12px;font-weight:bold;color:#888;margin-bottom:12px;text-transform:uppercase;letter-spacing:.5px}\n.row2{display:grid;grid-template-columns:1fr 1fr;gap:12px}\n.row3{display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px}\nlabel.field{display:flex;flex-direction:column;gap:4px}\n.lbl{font-size:11px;font-weight:bold;color:#555}\n.hint{font-size:10px;color:#aaa;font-weight:normal}\ninput[type=text],input[type=number]{padding:7px 10px;border:1px solid #d0d7de;border-radius:5px;font-size:13px;transition:border .2s}\ninput:focus{outline:none;border-color:#9b59b6;box-shadow:0 0 0 2px rgba(155,89,182,.15)}\n.checks{display:flex;gap:14px;margin-top:4px}\n.checks label{display:flex;align-items:center;gap:5px;font-size:13px;cursor:pointer}\n.radios{display:flex;gap:14px;margin-top:4px;flex-wrap:wrap}\n.radios label{display:flex;align-items:center;gap:5px;font-size:12px;cursor:pointer}\n.btn-row{display:flex;gap:8px;margin-top:14px}\n.btn{padding:7px 16px;border:none;border-radius:5px;cursor:pointer;font-size:12px;font-weight:bold}\n.btn-save{background:#27ae60;color:white}\n.btn-del{background:#e74c3c;color:white}\n.btn-imp{background:#e67e22;color:white}\n.btn-sync{background:#2980b9;color:white}\n/* Tariffa table */\n.tariff-table{width:100%;border-collapse:collapse;font-size:12px;margin-top:8px}\n.tariff-table th{background:#6c3483;color:white;padding:6px 8px;text-align:left}\n.tariff-table td{padding:5px 8px;border-bottom:1px solid #f0f0f0}\n.tariff-table tr:hover td{background:#f8f0ff}\n#status{font-size:11px;color:rgba(255,255,255,.8);margin-right:8px}\n" + "</style></head><body>" + "<div id='topbar'>" + "<h2>&#x1F69A; Gestione Vettori</h2>" + "<span id='status'></span>" + "<button class='tbtn' onclick='syncFromGist()' style='background:#2980b9'>&#x21C5; Sync Gist</button>" + "<button class='tbtn' onclick='window.close()' style='background:#7f8c8d'>Chiudi</button>" + "</div>" + "<div id='main'>" + "<div id='sidebar'>" + "<div id='sidebar-header'>VETTORI <button id='btn-nuovo' onclick='nuovoVettore()'>+ Nuovo</button></div>" + "<div id='vlist'></div>" + "</div>" + "<div id='editor' class='empty'><span>Seleziona un vettore dalla lista</span></div>" + "</div>" + "<script>" + "\nvar _GID = ''+GIST_ID+'';\nvar _TK  = ''+LS_TOKEN+'';\nvar _REG_FILE = ''+GIST_FILE_VETTORI_REG+'';\nvar _LS_REG  = ''+LS_VETTORI_REG+'';\nvar _SEED = [{\"id\": \"gavi\", \"nome\": \"Gavi\", \"porti\": [\"ITSPE\"], \"tipo\": \"localita\", \"add\": {}}, {\"id\": \"paglianiti\", \"nome\": \"Paglianiti\", \"porti\": [\"ITSPE\"], \"tipo\": \"localita\", \"add\": {}}, {\"id\": \"romeo\", \"nome\": \"Romeo\", \"porti\": [\"ITSPE\"], \"tipo\": \"localita\", \"add\": {}}, {\"id\": \"doublev\", \"nome\": \"Double V\", \"porti\": [\"ITSPE\", \"ITLIV\"], \"tipo\": \"localita\", \"add\": {}}, {\"id\": \"cfidenza\", \"nome\": \"C.Fidenza\", \"porti\": [\"ITSPE\", \"ITLIV\"], \"tipo\": \"localita\", \"add\": {}}, {\"id\": \"medlog\", \"nome\": \"Medlog\", \"porti\": [\"ITSPE\", \"ITLIV\"], \"tipo\": \"localita\", \"add\": {}}, {\"id\": \"trg\", \"nome\": \"TRG\", \"porti\": [\"ITSPE\"], \"tipo\": \"localita\", \"add\": {}}, {\"id\": \"truckrail\", \"nome\": \"Truck Rail\", \"porti\": [\"ITSPE\"], \"tipo\": \"km_nuovo\", \"add\": {}}, {\"id\": \"baccetti\", \"nome\": \"Baccetti\", \"porti\": [\"ITSPE\", \"ITLIV\"], \"tipo\": \"localita\", \"add\": {}}, {\"id\": \"silt\", \"nome\": \"Silt\", \"porti\": [\"ITSPE\"], \"tipo\": \"localita\", \"add\": {}}, {\"id\": \"gmt\", \"nome\": \"GMT\", \"porti\": [\"ITLIV\"], \"tipo\": \"localita\", \"add\": {}}, {\"id\": \"martelli\", \"nome\": \"Martelli\", \"porti\": [\"ITLIV\"], \"tipo\": \"localita\", \"add\": {}}, {\"id\": \"rat\", \"nome\": \"Rat\", \"porti\": [\"ITLIV\"], \"tipo\": \"localita\", \"add\": {}}];\n\nvar _registry = [];   // array vettori\nvar _selected = null; // id vettore selezionato\nvar _tariffe  = {};   // {id: [rows]}\n\n// ── Init ───────────────────────────────────────────────────────────\nfunction init() {\n  try {\n    var raw = localStorage.getItem(_LS_REG);\n    if (raw) { var d = JSON.parse(raw); _registry = d.vettori || []; _tariffe = d.tariffe || {}; }\n  } catch(e) {}\n  if (!_registry.length) { _registry = _SEED; }\n  renderSidebar();\n  document.getElementById('editor').className = 'empty';\n  document.getElementById('editor').innerHTML = '<span>Seleziona un vettore dalla lista</span>';\n}\n\n// ── Sidebar ────────────────────────────────────────────────────────\nfunction renderSidebar() {\n  var html = '';\n  _registry.forEach(function(v) {\n    var badges = '';\n    if (v.porti.indexOf('ITSPE') >= 0) badges += '<span class=\"badge badge-spe\">SPE</span>';\n    if (v.porti.indexOf('ITLIV') >= 0) badges += '<span class=\"badge badge-liv\">LIV</span>';\n    if (v.tipo !== 'localita') badges += '<span class=\"badge badge-km\">KM</span>';\n    html += '<div class=\"vitem' + (_selected===v.id?' active':'') + '\" onclick=\"selectV(' + JSON.stringify(v.id) + ')\">'\n      + '<span class=\"vname\">' + esc(v.nome) + '</span>'\n      + '<div class=\"vbadges\">' + badges + '</div></div>';\n  });\n  document.getElementById('vlist').innerHTML = html;\n}\n\n// ── Editor ─────────────────────────────────────────────────────────\nfunction selectV(id) {\n  _selected = id;\n  renderSidebar();\n  var v = _registry.find(function(x){return x.id===id;});\n  if (!v) return;\n  renderEditor(v);\n}\n\nfunction renderEditor(v) {\n  var add = v.add || {};\n  var rows = _tariffe[v.id] || [];\n  var isKm = v.tipo !== 'localita';\n\n  var addFields = [\n    {k:'fuel_perc',      l:'Fuel %',             u:'%', h:'Sul costo base'},\n    {k:'hc',             l:'HC',                  u:'€'},\n    {k:'adr',            l:'ADR',                 u:'€'},\n    {k:'vgm',            l:'VGM',                 u:'€'},\n    {k:'extra_stop',     l:'Extra Stop',           u:'€'},\n    {k:'notte',          l:'Sosta Notte',          u:'€'},\n    {k:'congestion_liv', l:'Congestion Livorno',   u:'€'},\n    {k:'congestion_spe', l:'Congestion La Spezia', u:'€'},\n    {k:'reefer_perc',    l:'Reefer %',             u:'%', h:'Su base+fuel'}\n  ];\n\n  var addHtml = '<div class=\"row3\">';\n  addFields.forEach(function(f) {\n    addHtml += '<label class=\"field\"><span class=\"lbl\">' + f.l\n      + (f.h ? ' <span class=\"hint\">— ' + f.h + '</span>' : '') + '</span>'\n      + '<div style=\"display:flex;align-items:center;gap:5px\">'\n      + '<input type=\"number\" class=\"add-field\" data-key=\"' + f.k + '\" min=\"0\" step=\"0.01\" value=\"' + (add[f.k]||'') + '\" placeholder=\"0\" style=\"width:80px\">'\n      + '<span style=\"font-size:12px;color:#888\">' + f.u + '</span></div></label>';\n  });\n  addHtml += '</div>';\n\n  // Tabella tariffe (prime 20 righe)\n  var tariffHtml = '';\n  if (rows.length) {\n    if (isKm) {\n      tariffHtml = '<table class=\"tariff-table\"><thead><tr><th>KM da</th><th>KM a</th><th>20\\'</th><th>40\\'</th></tr></thead><tbody>';\n      rows.slice(0,20).forEach(function(r) {\n        tariffHtml += '<tr><td>' + (r.km_da||'') + '</td><td>' + (r.km_a||'') + '</td><td>€' + (r.c20||'') + '</td><td>€' + (r.c40||'') + '</td></tr>';\n      });\n    } else {\n      tariffHtml = '<table class=\"tariff-table\"><thead><tr><th>Localit&#xE0;</th><th>Prov</th><th>CAP</th><th>KM</th><th>20\\'</th><th>40\\'</th></tr></thead><tbody>';\n      rows.slice(0,20).forEach(function(r) {\n        tariffHtml += '<tr><td>' + esc(r.localita||'') + '</td><td>' + (r.prov||'') + '</td><td>' + (r.cap||'') + '</td><td>' + (r.km||'') + '</td><td>€' + (r.c20||'') + '</td><td>€' + (r.c40||'') + '</td></tr>';\n      });\n    }\n    tariffHtml += '</tbody></table>';\n    if (rows.length > 20) tariffHtml += '<div style=\"font-size:11px;color:#888;padding:6px 0\">... e altre ' + (rows.length-20) + ' righe</div>';\n  } else {\n    tariffHtml = '<div style=\"color:#bbb;font-size:12px;padding:8px 0\">Nessun tariffario caricato. Usa <b>Carica Excel</b>.</div>';\n  }\n\n  var ed = document.getElementById('editor');\n  ed.className = '';\n  ed.innerHTML =\n    '<div class=\"card\">'\n    + '<div class=\"card-title\">Anagrafica</div>'\n    + '<div class=\"row2\" style=\"margin-bottom:10px\">'\n    + '<label class=\"field\"><span class=\"lbl\">Nome vettore</span>'\n    + '<input type=\"text\" id=\"v-nome\" value=\"' + esc(v.nome) + '\"></label>'\n    + '<div>'\n    + '<div class=\"lbl\" style=\"margin-bottom:6px\">Tipo tariffario</div>'\n    + '<div class=\"radios\">'\n    + '<label><input type=\"radio\" name=\"v-tipo\" value=\"localita\"' + (v.tipo==='localita'?' checked':'') + '> Per Localit&#xE0;</label>'\n    + '<label><input type=\"radio\" name=\"v-tipo\" value=\"km_nuovo\"' + (v.tipo==='km_nuovo'?' checked':'') + '> KM (nuovo)</label>'\n    + '<label><input type=\"radio\" name=\"v-tipo\" value=\"km_vecchio\"' + (v.tipo==='km_vecchio'?' checked':'') + '> KM (vecchio)</label>'\n    + '</div></div></div>'\n    + '<div class=\"lbl\" style=\"margin-bottom:6px\">Porti operativi</div>'\n    + '<div class=\"checks\" style=\"margin-bottom:12px\">'\n    + '<label><input type=\"checkbox\" id=\"v-spe\"' + (v.porti.indexOf('ITSPE')>=0?' checked':'') + '> La Spezia</label>'\n    + '<label><input type=\"checkbox\" id=\"v-liv\"' + (v.porti.indexOf('ITLIV')>=0?' checked':'') + '> Livorno</label>'\n    + '</div>'\n    + '<div class=\"btn-row\">'\n    + '<button class=\"btn btn-save\" onclick=\"saveVettore()\">&#x1F4BE; Salva anagrafica</button>'\n    + '<button class=\"btn btn-del\" onclick=\"deleteVettore()\">&#x1F5D1; Elimina vettore</button>'\n    + '</div></div>'\n\n    + '<div class=\"card\">'\n    + '<div class=\"card-title\">Addizionali</div>'\n    + addHtml\n    + '<div class=\"btn-row\">'\n    + '<button class=\"btn btn-save\" onclick=\"saveAddizionali()\">&#x1F4BE; Salva addizionali</button>'\n    + '</div></div>'\n\n    + '<div class=\"card\">'\n    + '<div class=\"card-title\">Tariffario &nbsp;<span style=\"font-weight:normal;color:#aaa\">(' + rows.length + ' righe)</span>'\n    + '<input type=\"file\" id=\"inp-xls\" accept=\".xlsx,.xls\" style=\"display:none\" onchange=\"importExcel(this)\">'\n    + '</div>'\n    + '<div class=\"btn-row\" style=\"margin-bottom:10px\">'\n    + '<button class=\"btn btn-imp\" onclick=\"document.getElementById(\\'inp-xls\\').click()\">&#x1F4C2; Carica Excel</button>'\n    + (rows.length ? '<button class=\"btn\" style=\"background:#7f8c8d;color:white\" onclick=\"clearTariffe()\">&#x2716; Svuota tariffe</button>' : '')\n    + '</div>'\n    + tariffHtml\n    + '</div>';\n}\n\n// ── CRUD Vettori ───────────────────────────────────────────────────\nfunction saveVettore() {\n  var v = _registry.find(function(x){return x.id===_selected;});\n  if (!v) return;\n  v.nome = document.getElementById('v-nome').value.trim() || v.nome;\n  v.tipo = document.querySelector('input[name=\"v-tipo\"]:checked').value;\n  v.porti = [];\n  if (document.getElementById('v-spe').checked) v.porti.push('ITSPE');\n  if (document.getElementById('v-liv').checked) v.porti.push('ITLIV');\n  saveToStorage();\n  pushRegistry();\n  renderSidebar();\n  setStatus('Anagrafica salvata \\u2713');\n}\n\nfunction saveAddizionali() {\n  var v = _registry.find(function(x){return x.id===_selected;});\n  if (!v) return;\n  v.add = {};\n  document.querySelectorAll('.add-field').forEach(function(el) {\n    v.add[el.dataset.key] = el.value.trim();\n  });\n  saveToStorage();\n  pushRegistry();\n  setStatus('Addizionali salvati \\u2713');\n}\n\nfunction deleteVettore() {\n  if (!confirm('Eliminare questo vettore?')) return;\n  _registry = _registry.filter(function(x){return x.id!==_selected;});\n  delete _tariffe[_selected];\n  _selected = null;\n  saveToStorage();\n  pushRegistry();\n  renderSidebar();\n  document.getElementById('editor').className = 'empty';\n  document.getElementById('editor').innerHTML = '<span>Seleziona un vettore dalla lista</span>';\n  setStatus('Vettore eliminato');\n}\n\nfunction nuovoVettore() {\n  var nome = prompt('Nome del nuovo vettore:');\n  if (!nome || !nome.trim()) return;\n  var id = nome.trim().toLowerCase().replace(/[^a-z0-9]/g,'_') + '_' + Date.now().toString(36);\n  _registry.push({id:id, nome:nome.trim(), porti:['ITSPE'], tipo:'localita', add:{}});\n  saveToStorage();\n  renderSidebar();\n  selectV(id);\n}\n\nfunction clearTariffe() {\n  if (!confirm('Svuotare le tariffe di questo vettore?')) return;\n  _tariffe[_selected] = [];\n  saveToStorage();\n  selectV(_selected);\n  setStatus('Tariffe svuotate');\n}\n\n// ── Import Excel ───────────────────────────────────────────────────\nfunction importExcel(input) {\n  var f = input.files[0]; if (!f) return;\n  var v = _registry.find(function(x){return x.id===_selected;});\n  var isKm = v && v.tipo !== 'localita';\n  var reader = new FileReader();\n  reader.onload = function(ev) {\n    try {\n      var wb = XLSX.read(new Uint8Array(ev.target.result), {type:'array'});\n      var rows = [];\n      wb.SheetNames.forEach(function(sn) {\n        var rr = XLSX.utils.sheet_to_json(wb.Sheets[sn], {defval:'', raw:true});\n        rr.forEach(function(r) {\n          var row = {};\n          Object.keys(r).forEach(function(k){ row[k.trim()] = r[k]; });\n          if (isKm) {\n            // Colonne attese: KM_DA (o KM da), KM_A (o KM a), C20, C40\n            var kd = row['KM_DA']||row['KM da']||row['km_da']||row['Da']||'';\n            var ka = row['KM_A'] ||row['KM a'] ||row['km_a'] ||row['A'] ||'';\n            var c2 = row['C20']  ||row['c20']  ||row['costo_20']||'';\n            var c4 = row['C40']  ||row['c40']  ||row['costo_40']||'';\n            if (kd!=='' && ka!=='') rows.push({km_da:String(kd),km_a:String(ka),c20:String(c2),c40:String(c4)});\n          } else {\n            var cap  = String(row['CAP']||row['cap']||'').trim();\n            var prov = String(row['Prov']||row['prov']||'').trim();\n            var loc  = String(row['Localita']||row['localita']||row['Localit\\u00e0']||'').trim();\n            var km   = String(row['KM']||row['km']||'').trim();\n            var c2   = String(row['C20']||row['c20']||row['costo_20']||'').trim();\n            var c4   = String(row['C40']||row['c40']||row['costo_40']||'').trim();\n            if (loc) rows.push({cap:cap,prov:prov,localita:loc,km:km,c20:c2,c40:c4});\n          }\n        });\n      });\n      if (!rows.length) { alert('Nessuna riga trovata.\\nColonne attese: ' + (isKm?'KM_DA, KM_A, C20, C40':'CAP, Prov, Localita, KM, C20, C40')); return; }\n      _tariffe[_selected] = rows;\n      saveToStorage();\n      pushTariffe(_selected);\n      selectV(_selected);\n      setStatus('Importate ' + rows.length + ' tariffe \\u2713');\n    } catch(e) { alert('Errore: ' + e.message); }\n  };\n  reader.readAsArrayBuffer(f);\n  input.value = '';\n}\n\n// ── Storage + Gist ─────────────────────────────────────────────────\nfunction saveToStorage() {\n  try { localStorage.setItem(_LS_REG, JSON.stringify({vettori:_registry, tariffe:_tariffe})); } catch(e) {}\n  if (window.opener && window.opener._gccOnVettoriSaved)\n    try { window.opener._gccOnVettoriSaved({vettori:_registry, tariffe:_tariffe}); } catch(e) {}\n}\n\nfunction pushRegistry() {\n  var tok = localStorage.getItem(_TK); if (!tok) return;\n  var content = JSON.stringify({vettori:_registry, updated_at:new Date().toISOString()}, null, 2);\n  fetch('https://api.github.com/gists/' + _GID, {\n    method:'PATCH',\n    headers:{'Authorization':'token '+tok,'Content-Type':'application/json'},\n    body: JSON.stringify({files:{[_REG_FILE]:{content:content}}})\n  }).then(function(r){ setStatus(r.ok ? 'Gist aggiornato \\u2713' : '\\u26A0 Gist errore '+r.status); })\n    .catch(function(e){ setStatus('\\u26A0 '+e.message); });\n}\n\nfunction pushTariffe(id) {\n  var tok = localStorage.getItem(_TK); if (!tok) return;\n  var v = _registry.find(function(x){return x.id===id;}); if (!v) return;\n  var fname = 'tcp_vettore_' + id + '.json';\n  var content = JSON.stringify({id:id, nome:v.nome, tipo:v.tipo, porti:v.porti, add:v.add||{}, rows:_tariffe[id]||[], updated_at:new Date().toISOString()}, null, 2);\n  var files = {}; files[fname] = {content:content};\n  var tok2 = localStorage.getItem(_TK);\n  fetch('https://api.github.com/gists/' + _GID, {\n    method:'PATCH',\n    headers:{'Authorization':'token '+tok2,'Content-Type':'application/json'},\n    body: JSON.stringify({files:files})\n  }).then(function(r){ setStatus(r.ok ? 'Tariffe Gist \\u2713' : '\\u26A0 '+r.status); })\n    .catch(function(e){ setStatus('\\u26A0 '+e.message); });\n}\n\nfunction syncFromGist() {\n  var tok = localStorage.getItem(_TK); if (!tok) { alert('Token non configurato.'); return; }\n  setStatus('\\u23F3 Sync...');\n  fetch('https://api.github.com/gists/' + _GID, {\n    headers:{'Authorization':'token '+tok,'Accept':'application/vnd.github.v3+json'}\n  }).then(function(r){return r.ok?r.json():null;})\n  .then(function(gd){\n    if (!gd) { setStatus('\\u26A0 Errore Gist'); return; }\n    var regFile = gd.files[_REG_FILE];\n    if (!regFile) { setStatus('\\u26A0 Registry non trovato'); return; }\n    var regData = regFile.truncated\n      ? fetch(regFile.raw_url).then(function(r){return r.json();})\n      : Promise.resolve(JSON.parse(regFile.content));\n    regData.then(function(rd){\n      _registry = rd.vettori || _registry;\n      // Carica tariffe di tutti i vettori\n      var proms = _registry.map(function(v){\n        var fname = 'tcp_vettore_' + v.id + '.json';\n        var f = gd.files[fname];\n        if (!f) return Promise.resolve(null);\n        var p = f.truncated\n          ? fetch(f.raw_url).then(function(r){return r.json();})\n          : Promise.resolve(JSON.parse(f.content));\n        return p.then(function(d){ if(d&&d.rows) { _tariffe[v.id]=d.rows; v.add=d.add||v.add||{}; } });\n      });\n      Promise.all(proms).then(function(){\n        saveToStorage();\n        renderSidebar();\n        if (_selected) selectV(_selected);\n        setStatus('Sync OK \\u2713 (' + _registry.length + ' vettori)');\n      });\n    });\n  }).catch(function(e){ setStatus('\\u26A0 '+e.message); });\n}\n\n// ── Helpers ────────────────────────────────────────────────────────\nfunction setStatus(msg) { document.getElementById('status').textContent = msg; }\nfunction esc(s){ var d=document.createElement('div');d.textContent=s;return d.innerHTML; }\n\nwindow.onload = function() {\n  // Carica XLSX\n  var s=document.createElement('script');\n  s.src='https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js';\n  s.onload=init;\n  document.head.appendChild(s);\n};\n" + "<\\/script>" + "</body></html>";

    var blob = new Blob([_html], {type:'text/html;charset=utf-8'});
    var url  = URL.createObjectURL(blob);
    var win  = window.open('', 'tcp_vettori', 'width=960,height=700,scrollbars=yes,resizable=yes');
