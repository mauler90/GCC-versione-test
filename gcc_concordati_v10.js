...
(function () {
  ...
  function apriGestioneVettori() {
    ...
    window._gccOnVettoriTariffeUpdated = function(id, rows) {
      _gcc_vettori_tariffe[id] = rows;
      console.log('[GCC] Tariffe vettore aggiornate in memoria: ' + id + ' (' + rows.length + ' righe)');
    };
  ...
  function apriPopup(risultati){
    ...
    // Prepopola gruppi nell'opener per il pannello vettori
    if (window._gccVettoriGroups) window._gccVettoriGroups = [];
    var _allGruppi = gruppiOrdine.map(function(k){ return gruppiMap[k]; }).concat(mGruppiM);
    if (window._gccVettoriGroups) window._gccVettoriGroups = _allGruppi;
      ...
      htmlTrovati+=
        ...
        '<td class="no-print" style="white-space:nowrap">'+
          '<button title="Confronta vettori" '+
            ...
            'data-gi="'+gi+'" '+
            'onclick="showVettoriByKey(this.dataset.gi)">&#x1F69A;<\/button>'+
        '</td>'+
        '<td class="no-print" style="white-space:nowrap">'+
