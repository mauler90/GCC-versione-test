...
(function () {
  ...
  function _calcolaVettore(v, parsed, porto, ct, kmCRT, isADR, kmBase) {
    ...
    if (v.tipo === 'localita') {
      ...
      var found = null, minRow = null;
      rows.forEach(function(r) {
        var kd = parseFloat(r.km_da || 0);
        var ka = parseFloat(r.km_a  || r.km_da || 999999);
        if (effectiveKm >= kd && effectiveKm <= ka) found = r;
        if (!minRow || kd < parseFloat(minRow.km_da || 0)) minRow = r;
      });
      // Se ancora nessun match, usa la riga col km_da minore
      if (!found && minRow) found = minRow;
  ...
  function apriPopup(risultati){
    ...
    var scriptData=
      ...
      'function showVettori(g){'+
      'if(!window.opener||!window.opener._gccCalcolaVettori){alert("Dati vettori non disponibili. Ricarica lo script.");return;}'+
      'if(_vettoriLoading){alert("Vettori in caricamento, attendi un momento e riprova.");return;}'+
      'if(!_vettoriCount){alert("Nessun vettore configurato. Usa Gestisci Vettori.");return;}'+
