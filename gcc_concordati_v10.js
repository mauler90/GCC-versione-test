...
(function () {
  ...
  var LS_CRT_ITLIV   = 'tcp_crt_ITLIV';
  var LS_CRT_ITSPE   = 'tcp_crt_ITSPE';
  ...
  function aggiornaStato() {
    ...
    try {
      var _totaleCRT = 0;
      ['tcp_crt_ITLIV','tcp_crt_ITSPE'].forEach(function(k){
        var r = localStorage.getItem(k);
        if (r) { var p = JSON.parse(r); _totaleCRT += (p.rows||[]).length; }
      });
      if (_totaleCRT > 0) infoBase = {rows: {length: _totaleCRT}};
      else {
        var rawBase = localStorage.getItem(LS_LISTINO_BASE);
        if (rawBase) infoBase = JSON.parse(rawBase);
      }
  ...
  function _readCrtRows() {
    var rows = [];
    try {
      ['ITLIV','ITSPE'].forEach(function(pc) {
        var raw = localStorage.getItem('tcp_crt_' + pc);
        if (raw) { var p = JSON.parse(raw); rows = rows.concat(_crtDecompress(p.rows || [])); }
      });
      if (!rows.length) {
        var rawLeg = localStorage.getItem(LS_LISTINO_BASE);
        if (rawLeg) rows = _crtDecompress(JSON.parse(rawLeg).rows || []);
      }
    } catch(e) {}
    return rows;
  }
  ...
  function apriTariffarioCRT() {
    ...
    try {
      var rawFn = localStorage.getItem('tcp_crt_ITLIV');
      if (!rawFn) rawFn = localStorage.getItem(LS_LISTINO_BASE);
      if (rawFn) { var pFn = JSON.parse(rawFn); importedFilename = pFn.filename || ''; }
    } catch(e) {}
    ...
    var scriptData =
      ...
      'var _LS_ITLIV="tcp_crt_ITLIV";'+
      'var _LS_ITSPE="tcp_crt_ITSPE";'+
      'var _GID="'+GIST_ID+'";'+
